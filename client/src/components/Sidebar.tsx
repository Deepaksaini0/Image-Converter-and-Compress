...
export function Sidebar({ options, setOptions, onProcess, isProcessing, fileCount }: SidebarProps) {
    ...
    <div className="h-full flex flex-col bg-card/50 border-r border-border backdrop-blur-sm">
      <div className="p-6 border-b border-border">
        <div className="flex items-center gap-3 text-primary mb-1">
          ...
          <h2 className="font-display font-bold text-lg tracking-tight">Configuration</h2>
        </div>
        <p className="text-sm text-muted-foreground">Adjust your conversion settings.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <div className="bg-secondary/30 rounded-xl p-4 border border-white/5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="smart-compression" className="cursor-pointer">Target Size</Label>
            </div>
            <Switch
              id="smart-compression"
              checked={!!options.targetSizeKB}
              onCheckedChange={(checked) => updateOption("targetSizeKB", checked ? 500 : undefined)}
            />
          </div>
          ...
          {options.targetSizeKB !== undefined && (
            <div className="pt-2 animate-in slide-in-from-top-2 fade-in duration-200">
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground font-medium">KB</span>
              </div>
        ...
        <div className="flex items-center justify-between p-1">
          <Label htmlFor="keep-metadata" className="cursor-pointer">Preserve Metadata</Label>
          <Switch
            id="keep-metadata"
        ...
        <div className="space-y-4 pt-4 border-t border-border/50">
          ...
          <Input 
            ...
            onChange={(e) => updateOption("watermarkText", e.target.value || undefined)}
          ...
          {options.watermarkText && (
            <div className="space-y-3 pt-2">
               <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-mono">{Math.round(options.watermarkOpacity * 100)}%</span>
              </div>
              <Slider
