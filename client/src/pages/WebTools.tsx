...
type ToolType = "css-minify" | "css-beautify" | "js-minify" | "js-beautify" | "html-minify" | "html-beautify";
...
export default function WebTools() {
  ...
  const downloadOutput = () => {
    if (!output) return;
    const ext = tool.startsWith("css") ? "css" : tool.startsWith("js") ? "js" : "html";
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Minify or Beautify CSS, JS, and HTML</p>
            </div>
          </div>
          ...
          <Card className="flex flex-col h-[600px]">
            <CardHeader className="flex flex-row items-center justify-between gap-4 space-y-0 pb-2">
              ...
              <Button onClick={handleProcess} disabled={!input.trim() || isProcessing} className="hover-elevate">
                {isProcessing ? "Processing..." : tool.includes("minify") ? <Zap className="h-4 w-4 mr-2" /> : <Wand2 className="h-4 w-4 mr-2" />}
